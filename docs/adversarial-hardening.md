...
- Drivers: locked embedded conformance plus `scripts/conformance.sh` against digest-pinned PostgreSQL 16/MySQL 8/SQL Server 2022 containers with bounded startup waits.
...
- DuckDB has no savepoints or `SET TRANSACTION`; SQLite has no `SET TRANSACTION`; SQL Server has savepoints but no `RELEASE`, and its `SET TRANSACTION ISOLATION LEVEL` and `SET IMPLICIT_TRANSACTIONS` change the whole session rather than one unit. Those forms are rejected during transaction preflight before earlier statements in the same script can run.
- SQL Server exposes no session read-only flag, so a read-only connection there is enforced solely by Tusk's uniform client statement guard rather than by the engine as well. It also offers no out-of-band cancel through the TDS client, so a running query cannot be interrupted; the UI reports a running timer instead of a Cancel that cannot work.
- SQL Server paging appends `OFFSET … ROWS FETCH NEXT … ROWS ONLY` to the user statement. Without a top-level `ORDER BY` the added `ORDER BY (SELECT NULL)` leaves page order engine-defined, so concurrent writes can skew a paged read exactly as they can on the other `LIMIT`/`OFFSET` engines. Statement shapes that cannot take the clause are read once under the ordinary result budget and labelled, never silently truncated.
