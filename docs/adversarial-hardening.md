...
- File import parses on the backend and never crosses IPC as a blob. Delimited/JSON sources cap at 2 GiB, xlsx at 64 MiB and 5,000,000 cells, and every source at 10,000,000 rows, 10,000 columns, 1,000,000 characters per field, and 16 MiB per JSON array element / NDJSON line. Options cap at one non-newline delimiter/quote/escape character, a 1 KiB NULL placeholder, 1,000,000 skipped rows, and a 200-byte sheet name; identifiers cap at 512 bytes. Text is strictly decoded (UTF-8 with an optional BOM, or Latin-1) and never degrades to replacement characters. Generated statements batch at 1,000 rows or 1 MiB, and the mapped column list is revalidated against the file's own header so a file changed between preview and run is refused. Multi-table export caps at 2,000 tables.
...
- One import is one transaction on every engine, but MySQL DDL implicitly commits: a create-and-load import against MySQL can leave the empty table behind when the load fails. Import therefore clears an existing table with `DELETE` rather than `TRUNCATE` on MySQL and SQLite. Import cancellation is a cooperative flag checked between batches, so an engine with no out-of-band cancel still rolls back promptly; an individual batch statement still runs to completion.
