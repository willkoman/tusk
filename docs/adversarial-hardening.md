...
- Backup covers at most 50,000 objects with 20,000 selection entries of 512 bytes each; the dump itself is unbounded (it is a backup) and is written to a sibling temp file, fsynced, and renamed only on success, so cancel or failure leaves the previous file intact. Restore reads at most 2 GiB, buffers at most 256 MiB for one statement or one `COPY` data block, and rejects a non-UTF-8 file. Tusk's own dumps reopen `COPY` blocks every 16 MiB so they always restore within that per-unit cap.
...
- Backup fidelity is bounded by each driver's DDL reconstruction, which is PostgreSQL-first: partitioning, inheritance, row-level security, grants, storage parameters, and tablespaces are out of scope everywhere; DuckDB dumps carry no `CREATE INDEX` (its catalog exposes only the table definition); MySQL's `SHOW CREATE TABLE` is unqualified, so a MySQL dump restores into the connected database. Only PostgreSQL dumps are snapshot-consistent (one read-only repeatable-read transaction); the other engines page table by table, so a dump taken during concurrent writes is not a single point in time. Verify a restore before relying on a dump as the only copy.
