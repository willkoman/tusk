...
const NAME_REQUIRED = "The table needs a name.";
...
export function CreateTableForm(props: {
  ...
  // A form the user has not typed into yet has nothing wrong with it: the
  // disabled primary and the required marker already say the name is needed.
  ...
  const problems = createMemo(() => {
    ...
    if (!name().trim()) out.unshift({ level: "error" as const, message: NAME_REQUIRED });
  ...
  const shownErrors = () => errors().filter((p) => nameTouched() || p.message !== NAME_REQUIRED);
  ...
  return (
    ...
    >
      ...
      <Show when={shownErrors().length}>
        <div class="field-error">{shownErrors()[0].message}</div>
      </Show>
      <Show when={note()}>
